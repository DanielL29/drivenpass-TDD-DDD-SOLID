...
describe("user entity", () => {
  it("should create an user entity", () => {
    ...
    const user = User.create(createUserProps, true);
  ...
  it("should throw an error if user[name] is empty", () => {
    ...
    expect(() => User.create(createUserProps, true)).toThrow(
  ...
  it("should throw an error if user[password] is less than 6 characters length", () => {
    const createUserProps = createFakeUser({ password: faker.lorem.word(5) });

    expect(() => User.create(createUserProps, true)).toThrow(
      "user[password] must have at least 6 characters length"
    );
  });
});
